...
def calculate_fibonaci_sequence(n):
    return n if n < 2 else calculate_fibonaci_sequence(n - 2) + calculate_fibonaci_sequence(n - 1)
...
class awesomeProcessor(threading.Thread):
    ...
    def run(self):
        while not self.event.is_set():
            ...
            self.counter += 1
            calculate_fibonaci_sequence(data_to_process.number)
            if self.queue.empty():
                self.event.set()
